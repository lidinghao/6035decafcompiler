...
class config:
  ...
  run_optimizer = True
  gcc_extra = [
      #   join(dirname(abspath(sys.argv[0])), "../lib/lib6035.a"),
      #   "-lpthread"
              ]
  pad = 20
  showscore=False
...
def codediff(a,b):
  ...
  b=b.replace('\r','').split('\n')
  if len(a)>len(b):
    return "%d asm lines removed (%.1f%%)" % (len(a)-len(b), 100.0*(len(a)-len(b))/float(len(a)))
  if len(a)>len(b):
    return "%d asm lines added (%.1f%%)" % (len(b)-len(a), -100.0*(len(a)-len(b))/float(len(a)))
  changed = len(filter(lambda x: x[0]!=x[1], zip(a,b)))
  return "%d asm lines changed (%.1f%%)" % (changed, -100.0*(changed)/float(len(a)))
...
def testcase_codegen(cmd, testname, i, correct):
  ...
  def dcfcompile(cmd):
    ...
    if subprocess.call(["gcc", "-o", bfile, filename]+config.gcc_extra)!=0 or not isfile(bfile):
    ...
    output = p.stdout.read()
    p.wait()
    return output, open(filename).read()
  ...
  def norm(s):
    ...
    for i, line in enumerate(lines):
      if "RUNTIME ERROR" in line.upper():
        lines = lines[0:i]
  ...
  try:
    output1, src1 =dcfcompile(cmd)
    r1 = diff_answer(testname,norm(correct), norm(output1))
  except Incorrect:
    return 0
  ...
  if config.run_optimizer:
    try:
      testname += "[opt]"
      output2, src2 =dcfcompile(cmd[0:-2] + ['-opt', 'all'] + cmd[-2:])
      r2 = diff_answer(testname, norm(correct), norm(output2))
    except Incorrect:
      return 0
    print testname.ljust(config.pad), codediff(src1,src2)
    return min(r1, r2)
  else:
    return r1
...
def main(filename, tmpdir):
  ...
  if config.dataflow_tests:
    testwrap('DATAFLOW PUBLIC', test_codegen, config.dataflow_public_dir)
    testwrap('DATAFLOW HIDDEN', test_codegen, config.dataflow_hidden_dir)
