...
public class ExplicitGlobalLoadOptimizer {
	...
	public void setDf(GlobalsDefDFAnalyzer df) {
		this.df = df;
	}
...
//			if (lStmt.getVariable().toString().equals("z")) {
//			System.out.println("+==========================================+");
//			System.out.println(df.getUniqueGlobals().get(methodName));
//			for (CFGBlock blk: this.mMap.get(methodName).getCfgBlocks()) {
//				if (df.getCfgBlocksState().containsKey(blk)) {
//					System.out.println(blk);
//					System.out.println(df.getCfgBlocksState().get(blk));
//					System.out.println();
//				}
//			}
//			System.out.println("+******************************************+");
//		}
	...
	private void insertLoad(CFGBlock block, LoadStmt loadStmt) {
		...
		for (int i = block.getStatements().size()-1; i >= 0; i--) {
			...
			if (stmt.getClass().equals(LabelStmt.class)) {
				...
				if (lStmt.getLabelString().matches(ExplicitGlobalLoadOptimizer.ForInitLabelRegex)) {
					index = i;
					break;
				}
			}
	...
	private CFGBlock getParentForInitBlock(CFGBlock block, int depth) {
		...
		while (!found) {
			...
			if (stmt.getClass().equals(LabelStmt.class)) {
				LabelStmt lStmt = (LabelStmt) stmt;
				if (lStmt.getLabelString().matches(ExplicitGlobalLoadOptimizer.ForTestLabelRegex)) {
					parent = parent.getPredecessors().get(0);
					found = true;
				}
			}
