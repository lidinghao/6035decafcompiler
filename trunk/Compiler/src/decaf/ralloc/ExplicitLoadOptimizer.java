...
public class ExplicitLoadOptimizer {
	...
	private ReachingGlobalDefinitionsAnalyzer df;
	...
	public ExplicitLoadOptimizer(HashMap<String, MethodIR> mMap) {
		this.df = new ReachingGlobalDefinitionsAnalyzer(mMap);
		this.mMap = mMap;
	...
	public ReachingGlobalDefinitionsAnalyzer getDf() {
		return df;
	}

	public void setDf(ReachingGlobalDefinitionsAnalyzer df) {
		this.df = df;
	}
			...
			for (int i = 0; i < block.getStatements().size(); i++) {
				...
				if (stmt.getClass().equals(LoadStmt.class)) { // Found load
					...
					if (prev.getClass().equals(LabelStmt.class)) {
						...
						if (lStmt.getLabelString().matches(ExplicitLoadOptimizer.ArrayPassLabelRegex)) {
							addBC = false;
						}
					}
...
//		}
	...
	private void insertLoad(CFGBlock block, LoadStmt loadStmt) {
		...
		for (int i = block.getStatements().size()-1; i >= 0; i--) {
			...
			if (stmt.getClass().equals(LabelStmt.class)) {
				...
				if (lStmt.getLabelString().matches(ExplicitLoadOptimizer.ForInitLabelRegex)) {
					index = i;
					break;
				}
			}
	...
	private CFGBlock getParentForInitBlock(CFGBlock block, int depth) {
		...
		while (!found) {
			...
			if (stmt.getClass().equals(LabelStmt.class)) {
				LabelStmt lStmt = (LabelStmt) stmt;
				if (lStmt.getLabelString().matches(ExplicitLoadOptimizer.ForTestLabelRegex)) {
					parent = parent.getPredecessors().get(0);
					found = true;
				}
			}
